...
func run(t: TestContext) -> void:
	...
	var peatpaw: CreatureInstance = DataRegistry.make_creature("peatpaw", 6)
	peatpaw.apply_damage(9)
	real_party.add_member(peatpaw)
	real_party.add_member(DataRegistry.make_creature("drippole", 4))
	...
	t.check_eq(restored_first.creature_id, "peatpaw", "roundtrip keeps creature id")
	t.check_eq(restored_first.level, 6, "roundtrip keeps level")
	t.check_eq(restored_first.hp, peatpaw.hp, "roundtrip keeps current HP")
	t.check_eq(restored_first.max_hp, peatpaw.max_hp, "stats are rebuilt from the definition")
	party.free()
	real_party.free()
	restored.free()
