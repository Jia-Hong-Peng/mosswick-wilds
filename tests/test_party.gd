...
func run(t: TestContext) -> void:
	...
	var mosshorn: CreatureInstance = DataRegistry.make_creature("mosshorn", 6)
	mosshorn.apply_damage(9)
	real_party.add_member(mosshorn)
	real_party.add_member(DataRegistry.make_creature("tidewing", 4))
	...
	t.check_eq(restored_first.creature_id, "mosshorn", "roundtrip keeps creature id")
	t.check_eq(restored_first.level, 6, "roundtrip keeps level")
	t.check_eq(restored_first.hp, mosshorn.hp, "roundtrip keeps current HP")
	t.check_eq(restored_first.max_hp, mosshorn.max_hp, "stats are rebuilt from the definition")
	party.free()
	real_party.free()
	restored.free()
